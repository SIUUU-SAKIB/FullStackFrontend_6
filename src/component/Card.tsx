...
const Card: React.FC<CardProps> = ({ parcel, approved, delivered, pending, meData, refetch, track, all }) => {
  const { setTrcNum } = useContext(contextApi) as ContextType;
  
  // Fix: Initialize with empty array if parcel.data[0] doesn't exist
  const [allParcel, setAllParcel] = useState<Parcel[]>(parcel.data[0] || []);
  const [deleteItem, { isLoading }] = useCancelParcelMutation();

  useEffect(() => {
    if (!parcel || parcel.data.length === 0 || !parcel.data[0]) return;

    const filteredParcels = parcel.data[0].filter((e: Parcel) => {
      if (approved) {
        // FIXED: Removed the unnecessary "in_route" comparison
  ...
  }, [approved, pending, delivered, parcel, track, all]);
  ...
  const cancelBtn = (trackingNumber: string) => {
    Swal.fire({
      ...
      try {
        await deleteItem({ trackingNumber }).unwrap();
        refetch();
      } catch (error) {
        console.log(error);
      }
  ...
  // Type guard to check if user has nested user object
  const isNestedUser = (user: any): user is { user: User } => {
    return user && user.user && typeof user.user === 'object';
  };
  ...
  const getUserRole = () => {
    ...
    if (isNestedUser(meData.currentUser)) {
      return meData.currentUser.user.role;
    } else {
      return (meData.currentUser as User).role;
    }
  };
      ...
      {allParcel.length > 0 ? (
        allParcel.map((e) => (
          <div
            key={e._id}
            className={`${
              e.currentStatus === "delivered" && "bg-zinc-300"
          ...
          >
            <div className="flex flex-col items-start justify-center py-2">
              ...
              <p className="text-center h-auto font-semibold">
                Tracking ID:{" "}
                <span onClick={() => setTrcNum(e.trackingNumber)} className="text-blue-700 cursor-pointer">
                  {e.trackingNumber}
                </span>
              </p>
            </div>
            ...
            <div className="border-t-1 border-black/10 pt-2">
              ...
              <p className="text-gray-600">
                ...
                {e.expectedDeliveryDate ? e.expectedDeliveryDate : ""}
              </p>
              <p className="text-gray-600">
        ...
        <h1 className="text-xl font-bold text-center text-white z-10 w-full">
          {userRole === "sender"
            ? "You don't have any parcels 😔"
            : "You don't have any incoming parcels 😔"}
        </h1>
      )}
    </>
  );
